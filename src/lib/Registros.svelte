<script>
	...
	const unSub = onSnapshot(
		collection(db, 'afis'), //consulta a la coleccion de tareas de firestore...
		(querySnpshot) => {
		...
		<table class="table m-0">
			...
			<tbody>
				{#each afis as afi}
					<tr>
						<td>
							<button on:click={deleteData(afi.id)} id="bo3"><ion-icon name="trash" /></button>
							<button on:click={editData(afi)} id="bo3"><ion-icon name="pencil" /></button>
						</td>
